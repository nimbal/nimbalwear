...
def process_subject_visit(study_dir, subject_id, visit, quiet=False):
    ...
    # file path constants
    device_list_path = os.path.join(study_dir, 'device_list.csv')
    nonwear_csv = os.path.join(study_dir, 'analyzed_data/nonwear/standard_nonwear_times.csv')

    # folder constants
    raw_data_dir = os.path.join(study_dir, 'raw_data')
    standard_device_dir = os.path.join(study_dir, 'processed_data/standard_device_edf')
    cropped_device_dir = os.path.join(study_dir, 'processed_data/cropped_device_edf')
    sensor_dir = os.path.join(study_dir, 'processed_data/sensor_edf')

    # read device list
    device_list_df = pd.read_csv(os.path.join(study_dir, device_list_path), dtype=str, )

    # search for files from subject visit
    subject_device_list_df = device_list_df.loc[(device_list_df['subject_id'] == subject_id) &
                                                (device_list_df['visit'] == visit)]

    # TEMP - SELECT DEVICE ROW
    # device_info = subject_device_list_df.loc[[5]]
    # device_info = device_info.to_dict('records')[0]
    ...
    for index, row in tqdm(subject_device_list_df.iterrows(), desc='Processing devices'):
        ...
        import_func = import_switch.get(row['device_type'], lambda: 'Invalid')
        sensors = sensors_switch.get(row['device_type'], lambda: 'Invalid')
        sensor_channels = sensor_channels_switch.get(row['device_type'], lambda: 'Invalid')

        # create all file path variables
        device_file_name = row['file_name']
        device_file_base = os.path.splitext(device_file_name)[0]

        device_raw_path = os.path.join(raw_data_dir, row['device_type'], device_file_name)

        device_edf_name = '.'.join([device_file_base, 'edf'])

        sensor_edf_names = ['.'.join(['_'.join([device_file_base, sensor]), 'edf']) for sensor in sensors]

        standard_device_path = os.path.join(standard_device_dir, row['device_type'], device_edf_name)
        cropped_device_path = os.path.join(cropped_device_dir, row['device_type'], device_edf_name)
        sensor_paths = [os.path.join(sensor_dir, row['device_type'], sensors[sen], sensor_edf_names[sen])
        ...
        nonwear_df = pd.read_csv(nonwear_csv, dtype=str)
        ...
        # get subject_id and device_location from device list / input file
