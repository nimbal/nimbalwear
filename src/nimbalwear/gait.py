...
def get_acc_data(accelerometer=None, axis=None, orient_signal=True, low_pass=True):
    ...
    def detect_fl_steps(device=None):
        '''
        Parameters
        ---
        ...
        if device.header['device_type'] == 'GNAC':
            ...
            def acc_step_detect_ssc(pushoff_df):
                """
                Detects the steps within the accelerometer data. Based on this paper:
                https://ris.utwente.nl/ws/portalfiles/portal/6643607/00064463.pdf
                """
                pushoff_df = get_pushoff_stats()
                pushoff_ind = push_off_detection(pushoff_df)
                ...
                for count, i in tqdm(enumerate(end_pushoff_ind), disable=self.quiet, total=len(end_pushoff_ind),
                                     leave=False,
                                     desc='%s: Step Detection' % self.label):
                    # check if next index within the previous detection
                    if end_i and i - self.pushoff_len < end_i:
            ...
            states = {1: 'stance', 2: 'push-off',
                           3: 'swing-up', 4: 'swing-down', 5: 'footdown'}
            state = states[1]
            pushoff_df = StepDetection.get_pushoff_stats(self.accel_path_or_obj,
                                                              start_end_times=[(self.start_dp, self.end_dp)],
                                                              quiet=self.quiet,
                                                              axis=self.axis) if pushoff_df is None else pushoff_df
            swing_peaks = []
